...
const validateName = (req, res, next) => {
  const { name } = req.body;
  if (!name) return res.status(400).json({ message: 'O campo "email" é obrigatório' });
  if (name.length <= 3) {
...
const validateAge = (req, res, next) => {
  const { age } = req.body;
  if (!age) return res.status(400).json({ message: 'O campo "email" é obrigatório' });
  if (Number(age) <= 18) {
