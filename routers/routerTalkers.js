...
router.put('/:id', checkToken, checkTalker, checkTalk, checkDate, checkRate, async (req, res) => {
  ...
  const currentTalkers = await getTalkers(database);
  const idUpdatedTalker = currentTalkers.findIndex((target) => target.id === parseInt(id, 10));
  currentTalkers[idUpdatedTalker] = { ...currentTalkers[idUpdatedTalker], ...newTalker };
  await updateTalkers(database, currentTalkers);
  res.status(200).json(currentTalkers[idUpdatedTalker]);
});
